...
### 7.2 커밋 메시지 규칙 (Conventional Commits)
- **한국어 커밋 메시지**: 모든 커밋 메시지는 한국어로 작성합니다.
- **타입 기반 커밋 메시지**:

| 타입 | 의미 | 언제 사용하는가? |
|------|------|-----------------|
| `feat` | 새로운 기능 | 새로운 기능을 추가할 때 (가장 중요) |
| `fix` | 버그 수정 | 코드의 버그를 수정했을 때 (가장 중요) |
| `chore` | 자질구레한 작업 | 빌드 스크립트, 패키지 매니저 설정, .gitignore 수정 등 |
| `docs` | 문서 | README.md, 주석 추가/수정 등 문서 관련 변경 |
| `style` | 코드 스타일 | 세미콜론, 들여쓰기 등 기능 변경 없는 스타일 수정 |
| `refactor` | 리팩토링 | 버그 수정/기능 추가 없이 코드 개선 (가독성, 성능) |
| `test` | 테스트 | 테스트 코드 추가, 수정, 삭제 |
| `ci` | CI/CD | CI/CD 설정 파일 수정 (Jenkinsfile, GitHub Actions 등) |
| `perf` | 성능 개선 | 코드 성능 향상 변경 |
| `revert` | 되돌리기 | 이전 커밋을 되돌릴 때 |
